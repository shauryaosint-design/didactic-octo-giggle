...
(function () {
  ...
  Promise.all([
    ...
    try {
      var result = Babel.transform(APP_CODE, { presets: [['react', { runtime: 'classic' }]] });
      var s = document.createElement('script');
      s.textContent = result.code;
      document.body.appendChild(s);
    } catch (err) {
      console.error(err);
      document.getElementById('root').innerHTML = '<div style="padding:60px;text-align:center;font-family:Inter,sans-serif;"><h2 style="color:#0f172a;">App failed to start</h2><p style="color:#64748b;">' + (err.message || err) + '</p></div>';
    }
  }).catch(function () {
